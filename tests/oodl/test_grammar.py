...
@entity Review: book: Book, reader: Reader, rating: int, comment: str;
deny unless Book as b or Reader as r
	sum of {review from Review as review} == 3
		or pay r.age at 5;""",
"""with Book() as b_0, Reader() as r_1, Review() as review_2:
	problem+=Assert(b_0, r_1).when(Sum({(review_2): (review_2)})==3).otherwise(r_1.age,5,0)""")
...
deny from Task as t, Project as p
	count of {t2.project from Task as t2 where t2.project.department=t.project.department} > 1
		or pay t.assigned_to.salary at 3;""",
"""with Task() as t_0, Project() as p_1, Task() as t2_2:
	problem+=Assert(False).when(t_0, p_1, Count({(t2_2.project): (t2_2, t2_2.project.department==t_0.project.department)})>1).otherwise(t_0.assigned_to.salary,3,0)""")