...
def test_build_tree_entity():
	input_str = """record Temperature: value: any;"""
	output_str = """
@atom
class Temperature:
	value: any
"""
	assert_in_transformation(input_str, output_str)


def test_comment():
	input_str = """
-- This is a comment.
record Temperature: value: any;
"""
	output_str = """
@atom
class Temperature:
...
"""
	output_str = """
with Node() as n_0, Edge() as e_1, Edge() as e2_2:
	problem+=When(e_1, e2_2, Literal(Atom(Predicate(f"{e_1.node1}=={n_0}")), True), Literal(Atom(Predicate(f"{e2_2.node1}=={n_0}")), True)).define(n_0)
"""
	assert_in_transformation(input_str, output_str)
...
define Person as pc
	...
	"""
	output_str = """
with Person() as pc_0, Car() as c_1, Ownership() as o_2:
	problem+=When(c_1, o_2, Literal(Atom(Predicate(f"{o_2.owner}=={pc_0}")), True), c_1.brand=='"Nissan"').define(pc_0)
	"""
	assert_in_transformation(input_str, output_str)
...
with Person() as pc_0, Ownership() as o_2, Car() as c_1:
	problem+=When(c_1, Sum({(1, o_2):(o_2, Literal(Atom(Predicate(f"{o_2.car}=={c_1}")), True))})>2).define(pc_0)
	"""
	assert_in_transformation(input_str, output_str)
...
define Person as pc
	from not Car as c
	where c.brand="Toyota" and c.model="Yaris"
	having count {o.owner from Ownership as o} > 2;
	"""
	output_str = """
with Person() as pc_0, Ownership() as o_2, Car() as c_1:
	problem+=When(~c_1, Count({(o_2.owner):(o_2)})>2, c_1.brand=='"Toyota"', c_1.model=='"Yaris"').define(pc_0)
	"""
	assert_in_transformation(input_str, output_str)
...
def test_build_tree_guess_1():
	input_str = """
record In_clause: clause: int, literal: int;
record Clause: id: int;
record Assignment: variable: Vars;
record WeightedClause: clause: int, literal: int;
record Vars: id: int;
...
guess from Vars as v
	...
	"""
	output_str = """
with Vars() as v_1, Assignment() as a_0:
	problem+=When(v_1).guess({a_0:(Literal(Atom(Predicate(f"{a_0.variable}=={v_1}")), True))})
	"""
	assert_in_transformation(input_str, output_str)
...
record Assign_: user:User, bit_rate: Bit_rate, sat_value: Sat;
...
guess from Town as t exactly 1 Path from Town where Path.index = 1 and Path.town = Town;

guess from Path as p
exactly 1 Path from Connected where Path.index = p.index + 1 and Path.town.id = Connected.b and Connected.a = p.town.id; 
	"""
	output_str = """
"""
	assert_in_transformation(input_str, output_str)
